...
const SOURCE = 'gangsamo';
const CAFE_ID = config.sources.gangsamo.cafeId;
const CAFE_NAME = 'dogpalza';
const BOARDS = config.sources.gangsamo.boards;
...
async function fetchViaNaverSearch(boardName, limit = 10) {
  const posts = [];
  try {
    const queryMap = { popular: '강아지', food: '사료 추천', health: '강아지 건강' };
    const query = queryMap[boardName] || '강아지';
    const url = `https://m.search.naver.com/search.naver?where=m&query=${encodeURIComponent(query + ' site:cafe.naver.com/dogpalza')}&sm=mob_hty`;
    const res = await fetch(url, { headers: HEADERS });
...
export async function collectGangsamo() {
...
if (process.argv[1] && import.meta.url.endsWith(process.argv[1].split('/').pop())) {
  collectGangsamo().catch(console.error);
}
